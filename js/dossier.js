				...
				xhr: function()
				{
					...
					xhr.upload.addEventListener("progress", function(evt){
						if (evt.lengthComputable) {  
							...
							$('#progressbar').html('Uploaden: '+ percentComplete.toFixed(0)+'%')
						}
					}, false);
					return xhr; 
				}, 
					...
					$('#bijlages').append('<li id="'+data._id['$id']+'" class="bijlage""><a href="/secretariaat/index.php/dossier/dossiers/files/'+data._id['$id']+'">'+data.name+'</a>&nbsp;&nbsp;<a id="del_bijlage" class="close_w" title="Bijlage verwijderen" href="#"><div class="g_close" alt="" style="display:inline-block"></div></a></li>');
					var bijlagen = $('input[name="bijlagen"]').val();
					if(bijlagen == ""){
						$('input[name="bijlagen"]').val(data._id['$id']);
					} else {
						$('input[name="bijlagen"]').val(bijlagen+', '+data._id['$id']);
					}
					$('#progressbar').html('');
				});	
        }  		
