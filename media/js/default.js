$(document).ready(function(){		
	...
	$(document).on('click', '#gen_password', function(event){
		...
		$('input[name=new_password], input[name=repeated_new_password]').val(random_password);
	});
	...
	$(document).on('click', 'a.logoutGoogle', function(event){
		event.preventDefault();
		signOutCallback(this.href);
	});
	$(document).on('change', '#change_order', function(event){
		user_list(1);
	});
	$(document).on('click', '.custom_pager', function(event){
		var page = $(this).attr('id');
		var page_array = page.split('_');
		user_list(page_array[1]);
	});
	$(document).on('click', '#page_prev', function(event){
		var active_page = $(this).parent().siblings('.active').children('a').attr('id');
		var page_array = active_page.split('_');
		var current_page = 'page_'+page_array[1];
		var page = parseFloat(page_array[1])-1;
		var first_page = $('.custom_pager').first().attr('id');
		if (first_page != current_page){
			user_list(page);
		}
	});
	$(document).on('click', '#page_next', function(event){
		var active_page = $(this).parent().siblings('.active').children('a').attr('id');
		var page_array = active_page.split('_');
		var current_page = 'page_'+page_array[1];
		var page = parseFloat(page_array[1])+1;
		var last_page = $('.custom_pager').last().attr('id');
		if (last_page != current_page){
			user_list(page);
		}
	});
	$(document).on('keyup', '#searchNaam', function(event){
		user_list(1);
	});
	function user_list(page){
		var search = $('#searchNaam').val();
		var order = $('#change_order option:selected').val();			
		$.get('/secretariaat/index.php/ajax/user/userlist/'+order+'/'+page+'/'+search, function(json){
			$('#userTable').html(json.content);
			$('th.sort').removeClass('active_th');
			$('.custom_pager').parent('li').removeClass('active');			
			$('#page_'+page).parent('li').addClass('active');
			$('th.sort').each(function(index){
				if ($(this).data('order')===json.order){					
			...
			if(page=='0'|| page=='1'){
				page = 1;
				$('#page_prev').parents('li').addClass('disabled');
			} else {
				$('#page_prev').parents('li').removeClass('disabled');
			}
			if (last_page == 'page_'+page){
				$('#page_next').parents('li').addClass('disabled');
			} else {
				$('#page_next').parents('li').removeClass('disabled');
			}
		});
	}
	...
	$(document).on('change', '#provincies', function(e){
	  $('#districten').children().children().unwrap();
	  
		var provincie = $(this).val();
		if ($(this).val() != ''){
		  $('#districten').children(':not(.empty)').each(function(index){
		    if ($(this).parent('span.toggleOption').length==0)
          $(this).wrap( '<span class="toggleOption" style="display: none;" />' );
		  });			  
			$('#districten').children().children('.'+provincie.replace(' ', '_')).unwrap();
			$('#districten').children().children('.empty').attr('selected', true);
		} else {
			$('#districten').children().children().unwrap();
		}
	});
	$('#provincies').trigger('change');
	...
	$(document).on('click', '.resetPassword', function(event){
		...
		var confirmMessage = confirm('Bent u zeker dat u het wachtwoord wilt resettten. Er zal een mail met het nieuwe wachtwoord naar de gebruiker gezonden worden.');
		$.get(href, function(json){
			if (json.success == 'yes'){
				alert('Het wachtwoord werd succesvol gereset. De gebruiker zal binnenkort een mail ontvangen met nieuwe login gegevens');
			}
		});
	});
	$(document).on('click', '.editUser', function(event){
		...
		$.get(href, function(json){		
			...
			history.pushState(json, title, href);
			$('#provincies').trigger('change');
		});
	});	
});

function signOutCallback(url) {
   	gapi.auth.signOut();
	window.location = url;	
}
...
function edit_profile(username, overlay) {	
	var title = 'Profiel bewerken';
	...
	$.get('/secretariaat/index.php/ajax/user/edit_account/'+username, function(json){
		...
		if(overlay){
			$('#overlay_content').hide().html(json.content).fadeIn('slow');
		} else {
			$('h1 .subtitle .name').html(json.title);
			$('h1 .subtitle .title').html(json.sub_title);
			$('#message').html(json.message);
		...
		var provincie = $('#provincies option:selected').val();
		if (provincie != ''){
