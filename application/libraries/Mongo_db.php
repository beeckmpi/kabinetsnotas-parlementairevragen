...
class Mongo_db {
	...
	*/
	...
	public function fullTextSearch($collection, $value="")
	{		
		try
		{
			return $this->db->command(array('text' => $collection, 'search' => $value, 'language' => 'dutch'));
		}
		catch (MongoCursorException $e)
		{
			show_error("search text into MongoDB failed: {$e->getMessage()}", 500);
		}
	}
	...
	*/
	 ...
	 public function get($collection = "")
	 {
	 	...
	 	if (isset($this->wheres['_id']) and ! ($this->wheres['_id'] instanceof MongoId))
		{
	 	...
	 	$documents = $this->db->{$collection}->find($this->wheres, $this->selects)->sort($this->sorts)->limit((int) $this->limit)->skip((int) $this->offset);
	...
	*/
	...
	public function gridFs_storeUpload($collection = "", $file = "", $filename){
		if (empty($collection))
		{
		...
		if (empty($file))
		{
			show_error("No file added to store");			
		}
		try
		{
			$metadata = time();
			$grid = $this->db->getGridFS($collection)->storeUpload($file, $filename);
		}
		catch (MongoCursorException $e)
		{
			show_error("Insert of file into MongoDB failed: {$e->getMessage()}", 500);
		}
		return $grid;
	}
	public function gridFs_storeThumbFile($collection = "", $file = "", $metadata = array()){
		if (empty($collection))
		{
			show_error("No Mongo collection selected to insert into", 500);
		}
		...
		{
			$metadata = time();
			$grid = $this->db->getGridFS($collection)->storeFile($file, array('metadata' => array('type' => 'thumb', 'date' => $metadata)));
		}
		catch (MongoCursorException $e)
		{
			show_error("Insert of file into MongoDB failed: {$e->getMessage()}", 500);
	...
	public function gridFs_storeFile($collection = "", $file = "", $metadata = array()){
		if (empty($collection))
		{
			show_error("No Mongo collection selected to insert into", 500);
		}
		...
		{
			$metadata = time();
			$grid = $this->db->getGridFS($collection)->storeFile($file, array('metadata' => array('type' => 'normal', 'date' => $metadata)));
		}
		catch (MongoCursorException $e)
		{
			show_error("Insert of file into MongoDB failed: {$e->getMessage()}", 500);
	...
	public function gridFs_getFile($collection = "", $filename){
		if (empty($collection))
		{
			show_error("No Mongo collection selected to insert into", 500);
		}
		...
		try
		{		
			$file = $this->db->getGridFS($collection)->findOne(array('_id' => new mongoID($filename)));
			return $file;
		}
		catch (MongoCursorException $e)
		{
	...
	public function gridFs_deleteFile($collection = "", $_id){
		if (empty($collection))
		{
			show_error("No Mongo collection selected to insert into", 500);
		}
		if (empty($_id))
		{
			show_error("No file was removed");			
		}
		try
		{		
			$file = $this->db->getGridFS($collection)->remove(array('_id' => new mongoID($_id)));
			return $file;
		}
		catch (MongoCursorException $e)
		{
